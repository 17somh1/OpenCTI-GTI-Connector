...
class GTIConnector:
    ...
    def convert_GTI_Report(self, data: dict, indicatorId: str, authorId: str):
    ...
    def _request_data_report_association_indicator(self, api_url: str, report_id: str, params=None):
        try:
            response = self.session.get(f"{api_url}/{report_id}/associations", params=params, verify=False)
            response.raise_for_status()
    ...
    def _collect_intelligence(self) -> list:
        ...
        for entity in entities.get("data", []):
            ...
            report = self.convert_GTI_Report(data=entity, indicatorId=indicator.id, authorId=author.id)
            stix_objects.append(report)

        stix_objects.append(stix2.TLP_WHITE)
    ...
    def process_message(self) -> None:
        ...
        try:
            ...
            self.helper.log_error(bundle.serialize(pretty=True))
